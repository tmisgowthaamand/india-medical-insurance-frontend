...
((ClientCapabilities3) => {
  ClientCapabilities3.LATEST = {
    textDocument: {
      completion: {
        completionItem: {
          documentationFormat: [import_vscode_languageserver_types.MarkupKind.Markdown, import_vscode_languageserver_types.MarkupKind.PlainText]
        }
      },
      hover: {
        contentFormat: [import_vscode_languageserver_types.MarkupKind.Markdown, import_vscode_languageserver_types.MarkupKind.PlainText]
      }
    }
  };
...
function getEntryMarkdownDescription(entry, settings) {
  ...
  if ((settings == null ? void 0 : settings.documentation) !== false) {
    ...
    if (typeof entry.description === "string") {
      ...
      result += entry.description.kind === import_vscode_languageserver_types.MarkupKind.Markdown ? entry.description.value : textToMarkedString(entry.description.value);
    }
    const browserLabel = getBrowserLabel(entry.browsers);
    if (browserLabel) {
...
var nls = __toESM(require("vscode-nls"));
...
var nls2 = __toESM(require("vscode-nls"));
...
var CSSHover = class {
  ...
  doHover(document, position, stylesheet, settings = this.defaultSettings) {
    function getRange2(node) {
      return import_vscode_languageserver_types.Range.create(document.positionAt(node.offset), document.positionAt(node.end));
    }
  ...
  doesSupportMarkdown() {
    if (!isDefined(this.supportsMarkdown)) {
      ...
      this.supportsMarkdown = hover && hover.contentFormat && Array.isArray(hover.contentFormat) && hover.contentFormat.indexOf(import_vscode_languageserver_types.MarkupKind.Markdown) !== -1;
    }
    return this.supportsMarkdown;
  }
};
...
var nls3 = __toESM(require("vscode-nls"));
...
var nls4 = __toESM(require("vscode-nls"));
...
var CSSCodeActions = class {
  ...
  doCodeActions(document, range, context, stylesheet) {
    return this.doCodeActions2(document, range, context, stylesheet).map((ca) => {
      ...
      return import_vscode_languageserver_types.Command.create(ca.title, "_css.applyCodeAction", document.uri, document.version, textDocumentEdit && textDocumentEdit.edits);
    });
  }
  ...
  getFixesForUnknownProperty(document, property, marker, result) {
    ...
    for (const candidate of candidates) {
      ...
      const edit = import_vscode_languageserver_types.TextEdit.replace(marker.range, propertyName2);
      const documentIdentifier = import_vscode_languageserver_types.VersionedTextDocumentIdentifier.create(document.uri, document.version);
      const workspaceEdit = { documentChanges: [import_vscode_languageserver_types.TextDocumentEdit.create(documentIdentifier, [edit])] };
      const codeAction = import_vscode_languageserver_types.CodeAction.create(title, workspaceEdit, import_vscode_languageserver_types.CodeActionKind.QuickFix);
...
var nls5 = __toESM(require("vscode-nls"));
...
var CSSValidation = class {
  ...
  doValidation(document, stylesheet, settings = this.settings) {
    ...
    function toDiagnostic(marker) {
      const range = import_vscode_languageserver_types.Range.create(
      ...
      return {
        ...
        severity: marker.getLevel() === 2 /* Warning */ ? import_vscode_languageserver_types.DiagnosticSeverity.Warning : import_vscode_languageserver_types.DiagnosticSeverity.Error,
        range
      };
    }
...
var cssProvider = (0, import_vscode_css_languageservice.getDefaultCSSDataProvider)();
var cssData = {
  version: 1.1,
...
function getSelectionRanges(document, positions, stylesheet) {
  function getSelectionRange(position) {
    ...
    for (let index = applicableRanges.length - 1; index >= 0; index--) {
      current = import_vscode_languageserver_types.SelectionRange.create(import_vscode_languageserver_types.Range.create(
        document.positionAt(applicableRanges[index][0]),
        document.positionAt(applicableRanges[index][1])
      ), current);
    }
    if (!current) {
      current = import_vscode_languageserver_types.SelectionRange.create(import_vscode_languageserver_types.Range.create(position, position));
    }
    return current;
  }
...
var nls6 = __toESM(require("vscode-nls"));
...
var nls7 = __toESM(require("vscode-nls"));
      ...
      for (const child of node.getSelectors().getChildren()) {
        if (child instanceof Selector) {
          if (child.getChildren().length === 1) {
            current.addSymbol(new Symbol2(child.getChild(0).getText(), void 0, child, 1 /* Rule */));
          }
        }
      }
    }
...
var nls8 = __toESM(require("vscode-nls"));

// src/vscode-css-languageservice/utils/resources.ts
var import_vscode_uri = require("vscode-uri");
function dirname(uriString) {
  return import_vscode_uri.Utils.dirname(import_vscode_uri.URI.parse(uriString)).toString(true);
}
function joinPath(uriString, ...paths) {
  return import_vscode_uri.Utils.joinPath(import_vscode_uri.URI.parse(uriString), ...paths).toString(true);
}
...
function pathToReplaceRange(valueBeforeCursor, fullValue, fullValueRange) {
  ...
  if (lastIndexOfSlash === -1) {
    ...
    replaceRange = import_vscode_languageserver_types.Range.create(startPos, endPos);
  }
  return replaceRange;
}
function createCompletionItem(name, isDir, replaceRange) {
  if (isDir) {
    ...
    return {
      ...
      kind: import_vscode_languageserver_types.CompletionItemKind.Folder,
      textEdit: import_vscode_languageserver_types.TextEdit.replace(replaceRange, escapePath(name)),
      command: {
        title: "Suggest",
    ...
    return {
      label: escapePath(name),
      kind: import_vscode_languageserver_types.CompletionItemKind.File,
      textEdit: import_vscode_languageserver_types.TextEdit.replace(replaceRange, escapePath(name))
    };
  }
}
...
function shiftPosition(pos, offset) {
  return import_vscode_languageserver_types.Position.create(pos.line, pos.character + offset);
}
function shiftRange(range, startOffset, endOffset) {
  const start = shiftPosition(range.start, startOffset);
  const end = shiftPosition(range.end, endOffset);
  return import_vscode_languageserver_types.Range.create(start, end);
}

// src/vscode-css-languageservice/services/cssCompletion.ts
var localize8 = nls8.loadMessageBundle();
var SnippetFormat = import_vscode_languageserver_types.InsertTextFormat.Snippet;
...
var CSSCompletion = class {
  ...
  doComplete(document, position, styleSheet, documentSettings2) {
    ...
    this.defaultReplaceRange = import_vscode_languageserver_types.Range.create(import_vscode_languageserver_types.Position.create(this.position.line, this.position.character - this.currentWord.length), this.position);
  ...
  getPropertyProposals(declaration, result) {
    ...
    properties.forEach((entry) => {
      ...
      const item = {
        ...
        tags: isDeprecated(entry) ? [import_vscode_languageserver_types.CompletionItemTag.Deprecated] : [],
        textEdit: import_vscode_languageserver_types.TextEdit.replace(range, insertText),
  ...
  getCompletionsForDeclarationValue(node, result) {
    ...
    if (entry) {
      ...
      for (const existingValue of existingValues.getEntries()) {
        result.items.push({
          label: existingValue,
          textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), existingValue),
    ...
    if (entry.values) {
      for (const value of entry.values) {
        ...
        const item = {
          ...
          tags: isDeprecated(entry) ? [import_vscode_languageserver_types.CompletionItemTag.Deprecated] : [],
          textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), insertString),
          sortText,
          kind: import_vscode_languageserver_types.CompletionItemKind.Value,
          insertTextFormat
        };
  ...
  getCSSWideKeywordProposals(entry, existingNode, result) {
    for (const keywords in cssWideKeywords) {
      result.items.push({
        ...
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), keywords),
        kind: import_vscode_languageserver_types.CompletionItemKind.Value
      });
    }
    for (const func in cssWideFunctions) {
      ...
      result.items.push({
        ...
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), insertText),
        kind: import_vscode_languageserver_types.CompletionItemKind.Function,
        insertTextFormat: SnippetFormat,
  ...
  getVariableProposals(existingNode, result) {
    ...
    for (const symbol of symbols) {
      ...
      const completionItem = {
        ...
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), insertText),
        kind: import_vscode_languageserver_types.CompletionItemKind.Variable,
        sortText: "z" /* Variable */
      };
      if (typeof completionItem.documentation === "string" && isColorString(completionItem.documentation)) {
        completionItem.kind = import_vscode_languageserver_types.CompletionItemKind.Color;
      }
  ...
  getVariableProposalsForCSSVarFunction(result) {
    ...
    for (const symbol of symbols) {
      if (startsWith(symbol.name, "--")) {
        const completionItem = {
          label: symbol.name,
          documentation: symbol.value ? getLimitedString(symbol.value) : symbol.value,
          textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(null), symbol.name),
          kind: import_vscode_languageserver_types.CompletionItemKind.Variable
        };
        if (typeof completionItem.documentation === "string" && isColorString(completionItem.documentation)) {
          completionItem.kind = import_vscode_languageserver_types.CompletionItemKind.Color;
    ...
    for (const name of allReferencedVariables.getEntries()) {
      if (startsWith(name, "--")) {
        const completionItem = {
          label: name,
          textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(null), name),
          kind: import_vscode_languageserver_types.CompletionItemKind.Variable
        };
        result.items.push(completionItem);
      }
        ...
        if (units2) {
          for (const unit of units2) {
            ...
            result.items.push({
              label: insertText,
              textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), insertText),
              kind: import_vscode_languageserver_types.CompletionItemKind.Unit
            });
          }
        }
      }
  ...
  getColorProposals(entry, existingNode, result) {
    for (const color in colors) {
      result.items.push({
        ...
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), color),
        kind: import_vscode_languageserver_types.CompletionItemKind.Color
      });
    }
  ...
  getPositionProposals(entry, existingNode, result) {
    for (const position in positionKeywords) {
      result.items.push({
        ...
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), position),
        kind: import_vscode_languageserver_types.CompletionItemKind.Value
      });
    }
    return result;
  ...
  getRepeatStyleProposals(entry, existingNode, result) {
    for (const repeat in repeatStyleKeywords) {
      result.items.push({
        ...
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), repeat),
        kind: import_vscode_languageserver_types.CompletionItemKind.Value
      });
    }
    return result;
  ...
  getLineStyleProposals(entry, existingNode, result) {
    for (const lineStyle in lineStyleKeywords) {
      result.items.push({
        ...
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), lineStyle),
        kind: import_vscode_languageserver_types.CompletionItemKind.Value
      });
    }
    return result;
  ...
  getLineWidthProposals(entry, existingNode, result) {
    for (const lineWidth of lineWidthKeywords) {
      result.items.push({
        label: lineWidth,
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), lineWidth),
        kind: import_vscode_languageserver_types.CompletionItemKind.Value
      });
    }
    return result;
  ...
  getGeometryBoxProposals(entry, existingNode, result) {
    for (const box in geometryBoxKeywords) {
      result.items.push({
        ...
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), box),
        kind: import_vscode_languageserver_types.CompletionItemKind.Value
      });
    }
    return result;
  ...
  getCompletionForTopLevel(result) {
    this.cssDataManager.getAtDirectives().forEach((entry) => {
      result.items.push({
        label: entry.name,
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(null), entry.name),
        documentation: getEntryDescription(entry, this.doesSupportMarkdown()),
        tags: isDeprecated(entry) ? [import_vscode_languageserver_types.CompletionItemTag.Deprecated] : [],
        kind: import_vscode_languageserver_types.CompletionItemKind.Keyword
      });
    });
  ...
  getCompletionsForSelector(ruleSet, isNested, result) {
    ...
    if (!isNested) {
      for (const entry of html5Tags) {
        result.items.push({
          label: entry,
          textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), entry),
          kind: import_vscode_languageserver_types.CompletionItemKind.Keyword
        });
      }
    ...
    this.styleSheet.accept((n) => {
      if (n.type === 5 /* SimpleSelector */ && n.length > 0) {
        ...
        if (selector.charAt(0) === "." && !visited[selector]) {
          ...
          result.items.push({
            label: selector,
            textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), selector),
            kind: import_vscode_languageserver_types.CompletionItemKind.Keyword
          });
        }
        return false;
  ...
  getCompletionForUriLiteralValue(uriLiteralNode, result) {
    ...
    if (!uriLiteralNode.hasChildren()) {
      ...
      range = import_vscode_languageserver_types.Range.create(emptyURIValuePosition, emptyURIValuePosition);
    } else {
  ...
  doesSupportMarkdown() {
    ...
    if (!isDefined(this.supportsMarkdown)) {
      ...
      this.supportsMarkdown = Array.isArray(documentationFormat) && documentationFormat.indexOf(import_vscode_languageserver_types.MarkupKind.Markdown) !== -1;
    }
    return this.supportsMarkdown;
  }
};
...
var nls9 = __toESM(require("vscode-nls"));
...
var _PostCSSCompletion = class extends CSSCompletion {
    ...
    if (parentType === 79 /* Forward */ || parentType === 77 /* Use */) {
      for (const p of _PostCSSCompletion.postcssModuleBuiltIns) {
        const item = {
          ...
          textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(importPathNode), `'${p.label}'`),
  ...
  getCompletionsForExtendsReference(_extendsRef, existingNode, result) {
    ...
    for (const symbol of symbols) {
      const suggest = {
        label: symbol.name,
        textEdit: import_vscode_languageserver_types.TextEdit.replace(this.getCompletionRange(existingNode), symbol.name),
        kind: import_vscode_languageserver_types.CompletionItemKind.Function
      };
...
var nls10 = __toESM(require("vscode-nls"));
...
var CSSNavigation = class {
  ...
  findSymbolInformations(document, stylesheet) {
    ...
    const addSymbolInformation = (name, kind, symbolNodeOrRange) => {
      ...
      const entry = {
        name,
        kind,
        location: import_vscode_languageserver_types.Location.create(document.uri, range)
      };
      result.push(entry);
    };
  ...
  findDocumentSymbols(document, stylesheet) {
    ...
    const addDocumentSymbol = (name, kind, symbolNodeOrRange, nameNodeOrRange, bodyNode) => {
      const range = symbolNodeOrRange instanceof Node ? getRange(symbolNodeOrRange, document) : symbolNodeOrRange;
      const selectionRange = (nameNodeOrRange instanceof Node ? getRange(nameNodeOrRange, document) : nameNodeOrRange) ?? import_vscode_languageserver_types.Range.create(range.start, range.start);
      const entry = {
        name,
        kind,
        range,
      ...
      if (node instanceof RuleSet) {
        for (const selector of node.getSelectors().getChildren()) {
          if (selector instanceof Selector) {
            const range = import_vscode_languageserver_types.Range.create(document.positionAt(selector.offset), document.positionAt(node.end));
            collect(selector.getText(), import_vscode_languageserver_types.SymbolKind.Class, range, selector, node.getDeclarations());
          }
        }
      } else if (node instanceof VariableDeclaration) {
        collect(node.getName(), import_vscode_languageserver_types.SymbolKind.Variable, node, node.getVariable(), void 0);
      } else if (node instanceof MixinDeclaration) {
        collect(node.getName(), import_vscode_languageserver_types.SymbolKind.Method, node, node.getIdentifier(), node.getDeclarations());
      } else if (node instanceof FunctionDeclaration) {
        collect(node.getName(), import_vscode_languageserver_types.SymbolKind.Function, node, node.getIdentifier(), node.getDeclarations());
      } else if (node instanceof Keyframe) {
        const name = localize10("literal.keyframes", "@keyframes {0}", node.getName());
        collect(name, import_vscode_languageserver_types.SymbolKind.Class, node, node.getIdentifier(), node.getDeclarations());
      } else if (node instanceof FontFace) {
        const name = localize10("literal.fontface", "@font-face");
        collect(name, import_vscode_languageserver_types.SymbolKind.Class, node, void 0, node.getDeclarations());
        ...
        if (mediaList instanceof Medialist) {
          ...
          collect(name, import_vscode_languageserver_types.SymbolKind.Module, node, mediaList, node.getDeclarations());
        }
      }
      return true;
    });
  ...
  getColorPresentations(document, stylesheet, color, range) {
    ...
    result.push({ label, textEdit: import_vscode_languageserver_types.TextEdit.replace(range, label) });
    if (color.alpha === 1) {
  ...
  doRename(document, position, newName, stylesheet) {
    const highlights = this.findDocumentHighlights(document, position, stylesheet);
    const edits = highlights.map((h) => import_vscode_languageserver_types.TextEdit.replace(h.range, newName));
    return {
...
function getHighlightKind(node) {
  if (node.type === 4 /* Selector */) {
    return import_vscode_languageserver_types.DocumentHighlightKind.Write;
  }
  if (node instanceof Identifier) {
  ...
  return import_vscode_languageserver_types.DocumentHighlightKind.Read;
}
...
var PostCSSNavigation = class extends CSSNavigation {
  ...
  async resolveRelativeReference(ref, documentUri, documentContext, isRawLink) {
    ...
    if (this.fileSystemProvider && target && isRawLink) {
      const parsedUri = import_vscode_uri2.URI.parse(target);
      try {
...
var documents = new import_node.TextDocuments(import_vscode_languageserver_textdocument2.TextDocument);
...
function getDocumentSettings(textDocument) {
  if (scopedSettingsSupport) {
    ...
    if (!promise) {
      ...
      promise = connection.sendRequest(import_node.ConfigurationRequest.type, configRequestParam).then((s) => s[0]);
